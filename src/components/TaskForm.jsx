...
const TaskForm = ({ className = 'taskForm', id = 'taskForm', setCreate }) => {
  ...
  return (
    <div className={`${className}`} id={`${id}`}>
      <div className={`${className}__form`} id={`${id}__form`}>
        ...
        <form className={`${className}__form__inputs`} id={`${id}__form__inputs`} >
          <input className={`${className}__form__inputs__title`} id={`${id}__form__inputs__title`} type="text" name="task" autoComplete="off" onChange={(e) => setTask(e.target.value)} required />
          <label className={`${className}__form__inputs__title__label`} id={`${id}__form__inputs__title__label`} htmlFor="task">task</label>
          <input className={`${className}__form__inputs__note`} id={`${id}__form__inputs__note`} type="text" name="note" autoComplete="off" onChange={(e) => setNote(e.target.value)} />
          ...
          <div className={`${className}__form__inputs__time`} id={`${id}__form__inputs__time`}>
            ...
            <div className={`${className}__form__inputs__time__input`}>
              <input className={`${className}__form__inputs__time__input__h`} id={`${id}__form__inputs__time__input__h`} type="number" min="0" max="5" placeholder="HH" name="hours" title="hours between 0 and 5" autoComplete="off" onInput={restrictNumberInput} onChange={(e) => setHours(e.target.value)} />
              <p>:</p>
              <input className={`${className}__form__inputs__time__input__m`} id={`${id}__form__inputs__time__input__m`} type="number" min="0" max="59" placeholder="MM" name="minutes" title="minutes between 0 and 59" autoComplete="off" onInput={restrictNumberInput} onChange={(e) => setMinutes(e.target.value)} />
            </div>
          </div>
        </form>
